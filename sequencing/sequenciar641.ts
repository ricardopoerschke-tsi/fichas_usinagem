...
type MedidasCantoneira641 = [number, number, number, number];

const DIMENSOES_INVALIDAS_641: Dimensoes641 = {
  comprimento: 0,
  largura: 0,
  espessura: 0,
  volume: 0,
...
function extrairDimensoes641(valor?: string): Dimensoes641 {
  ...
  if (!correspondencia) {
    return DIMENSOES_INVALIDAS_641;
  }
...
function temUrgenciaManual641(peca: Peca): boolean {
  return (
    peca.urgente === true ||
...
function temProcessoExterno641(peca: Peca): boolean {
  const observacoes = normalizarTexto(peca.observacoes);

  return PROCESSOS_EXTERNOS_641.some((processo) =>
    observacoes.includes(processo)
  );
}

function extrairMedidasCantoneira641(
  dimensoes?: string
): MedidasCantoneira641 | null {
  ...
  const medidas: MedidasCantoneira641 = [
...
function ehCantoneira(dimensoes?: string): boolean {
  return extrairMedidasCantoneira641(dimensoes) !== null;
}
...
function chaveOrdem641(peca: Peca, indiceOriginal: number): string {
...
function compararTexto641(a?: string, b?: string): number {
...
function compararItens641(
  ...
  const medidasCantoneiraA = extrairMedidasCantoneira641(a.peca.dimensoes);
  const medidasCantoneiraB = extrairMedidasCantoneira641(b.peca.dimensoes);
  const processoExternoA = temProcessoExterno641(a.peca);
  const processoExternoB = temProcessoExterno641(b.peca);
  ...
  const dimensoesA = extrairDimensoes641(a.peca.dimensoes);
  const dimensoesB = extrairDimensoes641(b.peca.dimensoes);
  ...
  const comparacaoDesenho = compararTexto641(a.peca.desenho, b.peca.desenho);

  if (comparacaoDesenho !== 0) {
    return comparacaoDesenho;
  }

  const comparacaoOrdemMes = compararTexto641(
    a.peca.ordemMes,
    b.peca.ordemMes
  );
...
export function sequenciar641(pecas: Peca[]): Peca[] {
  const grupos = new Map<string, GrupoOrdem641>();

  pecas.forEach((peca, indiceOriginal) => {
    const chave = chaveOrdem641(peca, indiceOriginal);
    const item = { peca, indiceOriginal };
    const grupo = grupos.get(chave);

    if (grupo) {
      grupo.itens.push(item);
      grupo.possuiUrgencia ||= temUrgenciaManual641(peca);
    ...
    grupos.set(chave, {
      itens: [item],
      possuiUrgencia: temUrgenciaManual641(peca),
  ...
  return [...grupos.values()]
    ...
    .flatMap((grupo) => {
      const cantoneirasComProcessoExterno = grupo.itens.some(
        ({ peca }) =>
          ehCantoneira(peca.dimensoes) && temProcessoExterno641(peca)
      );

      return [...grupo.itens]
        .sort((a, b) =>
          compararItens641(a, b, cantoneirasComProcessoExterno)
        )
        .map(({ peca }) => peca);
    });
}

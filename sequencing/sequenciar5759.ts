...
function getPrazoInternoTimestamp(peca: Peca) {
  return prazoInternoTimestamp(peca.prazo, DIAS_ANTECIPACAO_5759);
}
...
function getChaveGrupo(peca: Peca) {
  return [
    temProcessoExterno5759(peca) ? "processo-externo" : "normal",
...
function ajustarFerramentaPorQuantidade(pecas: Peca[]): Peca5759[] {
...
function temProcessoExterno5759(peca: Peca) {
  ...
  return PROCESSOS_EXTERNOS_5759.some((processo) => texto.includes(processo));
}
...
function getNivelUrgenciaGrupo5759(grupo: GrupoOrdem5759) {
...
function criarGruposOrdem(pecas: Peca5759[]) {
  const grupos = new Map<string, GrupoOrdem5759>();
  const chaves = new WeakMap<Peca5759, string>();

  pecas.forEach((peca, index) => {
    const ordem = getChaveOrdem(peca);
    const chave = ordem || `sem-ordem-${index}`;
    const urgente = temObservacaoUrgente5759(peca);
...
export function sequenciar5759(pecas: Peca[]) {
  ...
  return [...pecasComFerramenta].sort((a, b) => {
    ...
    const nivelUrgenciaA = getNivelUrgenciaGrupo5759(grupoOrdemA);
    const nivelUrgenciaB = getNivelUrgenciaGrupo5759(grupoOrdemB);
    ...
    const urgenciaPecaA = prioridadeUrgenciaPeca5759(a);
    const urgenciaPecaB = prioridadeUrgenciaPeca5759(b);

    if (urgenciaPecaA !== urgenciaPecaB) return urgenciaPecaA - urgenciaPecaB;

    const processoA = prioridadeProcessoExterno5759(a);
    const processoB = prioridadeProcessoExterno5759(b);
